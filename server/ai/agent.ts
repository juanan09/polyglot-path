...
/**
 * Output schema for the dialogue.
 */
export const DialogueSchema = genkitZ.object({
    ...
    grammar_score: genkitZ.number().default(1).describe('Grammar score of the player input, from 0.0 to 1.0.'),
...
/**
 * Creates the dialogue agent following the PerCLI pattern.
 */
export function createDialogueAgent(npcData: NPC, dialogueExamples: Dialogue[]) {
    // We return the pre-configured chat session with the prompt and NPC data
    // We use 'as any' because genkit/beta has some internal type incompatibilities between Prompts and Chat
